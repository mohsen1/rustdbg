...
You also have `rdbg`, a Rust debugger for this project (run `rdbg` for usage; read its
"When to reach for it" guidance first). Use it only once reading the code hasn't
localized the bug — not reflexively. Each launch rebuilds this large workspace, so a
wasted debugging detour is expensive.

Where it pays off here: a **wrong** or **extra** (false-positive) diagnostic. Most tsz
diagnostics funnel through one sink, `push_diagnostic`, so you can break there and trace
the emit *backward* to the deciding code instead of grepping:

  rdbg launch --cargo crates/tsz-checker --test {stem} --break-fn push_diagnostic -- {testfn}
  rdbg continue --until 'diag.code == <the wrong/extra code>'   # jump to the emit that matters
  rdbg bt                      # walk back to the exact decision that emitted it
  rdbg frame <n> ; rdbg vars   # inspect the types/flags that produced it

If the launch reports `push_diagnostic — bound, 0 hits`, this diagnostic uses a different
path: break on `emit_render_request`, or grep the emitted code for its emit site.

A **missing** diagnostic has nothing to trace — no emit fires. Don't hunt with repeated
launches: read to find *where the check should fire*, and only once you've located that
code, break there to see why its condition is false. If a couple of probes don't pin it,
go back to reading — the debugger can't show code that never ran.

Keep it cheap: one session with a few breakpoints beats re-launching; test a fix
hypothesis with `rdbg set --then continue` or by re-running the narrowed test, not by
re-launching after every edit."""
...
def run_agent(slot: Path, image: str, prompt: str, tpath: Path, env, model="opus", timeout=2700):
...
def one_case(slot: Path, image: str, case: dict, conds, results_path: Path, lock: threading.Lock, model: str):
    env = env_for(image)
    ...
    for i, cond in enumerate(conds):
        ...
        tpath = RESULTS / "transcripts" / f"{sha}-{cond}-{model}.jsonl"
        tpath.parent.mkdir(parents=True, exist_ok=True)
        info = run_agent(slot, image, prompt, tpath, env, model=model)
        subprocess.run([str(Path.home()/".local/bin/rdbg"), "down"], cwd=str(slot), capture_output=True)
        for pat in ("rdbg __daemon", "lldb-dap", "codelldb", "debugserver"):
            subprocess.run(["pkill", "-9", "-f", pat], capture_output=True)  # codelldb leaks ~20GB if not reaped
        passed = verify(slot, image, case, env) if not info["disk_killed"] else False
        row = {"case": sha, "model": model, "subject": case["subject"], "cond": cond,
               "baseline_red": base_red, "passed": passed, **info}
        with lock:
            existing = json.loads(results_path.read_text()) if results_path.exists() else []
            existing.append(row)
            results_path.write_text(json.dumps(existing, indent=2))
        print(f"[{model}/{sha}/{cond}] red={base_red} passed={passed} wall={info['wall_s']}s "
...
def main():
    ...
    ap.add_argument("--model", default="opus", help="claude model alias: opus | sonnet")
    a = ap.parse_args()
    ...
    print(f"{a.model} slot {a.slot} done ({len(idxs)} cases)")
